...
import { protect4 } from "../middleware/auth.js";
...
cloudinary.config({
 ...
 router.post("/register", asyncHandler(async(req, res) => {
    ...
    const existingtraining = await Training.findOne({ username });
    if (existingtraining ) {
      res.status(400);
 ...
 router.post("/login", asyncHandler(async(req, res) => {
    ...
    if (!username || !password) {
      ...
      const user = await Training.findOne({ username });
      if (!user) {
 ...
 router.get('/dashboard', protect4,  asyncHandler(async(req, res) => {
  ...
  router.put("/:id", protect4, asyncHandler(async(req, res) => {
    const {id} = req.params

    const exam = await Training.findById(id);
    if (!exam) {
      res.status(404);
    ...
    const updatedExam = await Training.findByIdAndUpdate(id, updates, {
        new: true,
      });
      if (!updatedExam) {
        res.status(500);
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const school = await Training.findById(id);
      if (!school) {
        res.status(404);
  ...
  router.get("/",  asyncHandler(async (req, res) => {
    try {
      const exam = await Training.find({});
      res.json(exam);
    } catch (err) {
...
router.post("/:id/shares", async(req, res) => {
  ...
  router.post("/:id/click", async (req, res) => {
    try {
      const { id } = req.params;
      const exam = await Training.findByIdAndUpdate(
        id,
        { $inc: { clicks: 1 } },
        { new: true }
      );
  ...
  router.get("/get-clicks", async (req, res) => {
    try {
      const exams = await Training.find({}, "exam clicks"); // Fetch school name and clicks only
  ...
  router.get("/atraining/:id", async (req, res) => {
  ...
  router.post("/:id/comments", async (req, res) => {
    ...
    try {
      const school = await Training.findById(req.params.id);
      if (!school) {
        return res.status(404).json({ message: "exam not found" });
      }
  ...
  router.get("/counttraining", async (req, res) => {
    try {
