...
const BookHomePage = () => {
    ...
    useEffect(() => {
        const fetchSchools = async () => {
            try {
              const response = await fetch(`${import.meta.env.VITE_APIB}`);
    ...
    const filteredCards = cardDetails.filter((card) => {
        ...
        const matchesSearchTerm = card.bookshopName
          ?.toLowerCase()
  ...
  return (
    <div>
      ...
      >
        Bookshop or Library Name
      </header>
      <div className="">
        <Link to="/bookshoplogin">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-w-7xl mx-auto mt-8 px-4">
        {filteredCards && filteredCards?.map((card, index) => (
          ...
          >
            ...
            />
            <div className="p-4">
              <h3 className="text-lg font-semibold mb-2">{card.BookOrLibrary}</h3>
              <p className="text-gray-600 mb-4">{card.bookshopName}</p>
              ...
              <Link to={`/bookshop/${card._id}`}>
                <button
                  className="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600"
                  onClick={async () => {
                    try {
                      await axios.post(
                        `${import.meta.env.VITE_APIB}/${card._id}/click`
                      );
