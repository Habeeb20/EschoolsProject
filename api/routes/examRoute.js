...
import { protect2 } from "../middleware/auth.js";
...
cloudinary.config({
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const existingexam = await Exam.findOne({ username });
      if (existingexam) {
        res.status(400);
  ...
  router.post("/login", asyncHandler(async(req, res) => {
    ...
    if (!username || !password) {
      ...
      const user = await Exam.findOne({ username });
      if (!user) {
  ...
  router.get('/dashboard', protect2,  asyncHandler(async(req, res) => {
  ...
  router.put("/:id", protect2, asyncHandler(async(req, res) => {
    const {id} = req.params
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const school = await Exam.findById(id);
      if (!school) {
        res.status(404);
...
router.get("/get-clicks/:id", async (req, res) => {
  ...
  router.get("/get-clicks", async (req, res) => {
    try {
      const exams = await Exam.find({}, "exam clicks"); // Fetch school name and clicks only
  ...
  router.get("/anexam/:id", async (req, res) => {
  ...
  router.post("/:id/comments", async (req, res) => {
    ...
    try {
      const school = await Exam.findById(req.params.id);
      if (!school) {
        return res.status(404).json({ message: "exam not found" });
      }
  ...
  router.get("/countexam", async (req, res) => {
    try {
